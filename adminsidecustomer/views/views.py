...
from adminsideserviceprovider.models import ServiceProvider, CustomerWithService, CustomerServiceContract, Hardware, \
	ServicePlanWithHardware, ContractbasedHardwarewithMAC
...
from adminnumberprovider.templatetags.check_number_functions import is_number_plus, check_number, check_number2, \
	is_rate_code_chart_plus, cost, plus_cost
...
def add(request):
	if request.method == 'GET':
		...
		return render(request, 'admin/users/add.html',
		              {'city': city, 'province': province, 'country': country, 'service_provider': service_provider,
		...
		if form.is_valid():
			...
			if Customer.objects.filter(pk=user.id).exists:
				...
				massege = render_to_string('admin/email_template/customer_data_mail.html',
				                           {'first_name': request.POST['first_name'],
				                            'last_name': request.POST['last_name'], 'msg': msg,
...
def details(request, id):
	if request.method == 'GET':
		...
		service_plan = CustomerWithService.objects.values('id', 'service_plan_id__title', 'service_price_actual',
		                                                  'service_price_retail', 'service_price_qty', 'plan_status',
		                                                  'plan_paid_status').filter(user_id=id, plan_status='y')
		monthly_total = 0
		...
		number_list = NumberProvinceCustomerMap.objects.values('number_id', 'number_id__did_id', 'number_id__number',
		                                                       ...
		                                                       'user_id__id', 'updated_at').filter(user_id=id).order_by(
			'updated_at')
		...
		card_list = CreditCard.objects.filter(user__id=id)
...
def view_hw_MAC(request, pk, customer_pk, id):
	if request.method == 'GET':
		hw_data = ContractbasedHardwarewithMAC.objects.values('id', 'hw_id', 'customer_id', 'hw_id__hw_title', 'type',
		                                                      'model', 'mac', 'sn', 'ver', 'usrn', 'passu', 'adusr',
		                                                      'adpass', 'dslusr',
		                                                      'dslpass', 'date_start', 'date_end', 'still_in_service',
		                                                      'device_buy', 'device_rental',
		                                                      'montly_rent').filter(hw_id=pk, customer_id=customer_pk,
		                                                                            pk=id)
		...
		return render(request, 'admin/users/hw/view.html',
		              {'hw_data': hw_data, 'pk': pk, 'customer_pk': customer_pk, 'data': data, 'id': id})
		...
		if ContractbasedHardwarewithMAC.objects.filter(hw_id=pk, customer_id=customer_pk, pk=id).exists():
			...
			if form.is_valid():
				...
				return HttpResponseRedirect(
					reverse('view_hw_MAC', kwargs={'pk': pk, 'customer_pk': customer_pk, 'id': id}))
			else:
				messages.add_message(request, messages.ERROR, form.errors)
				return HttpResponseRedirect(
					reverse('view_hw_MAC', kwargs={'pk': pk, 'customer_pk': customer_pk, 'id': id}))
...
def save_port_number(request):
	if request.is_ajax():
		...
		response_data = {}
		if status == 'success':
			NumberMNPtoCustomer.objects.create(number=mobile_number, user_id=customer_pk,
			                                   approve_upload_data=approve_upload_data)
...
def set_mnp_add(request):
	...
	if location_type == 'business':
		...
		querystring = {"token": "73ad3b26-88a2-4f59-90ae-9be6a256782d", "numbers": numbers,
		               "location_type": location_type, "business_contact": business_contact,
		               "business_name": business_name,
		               "account_number": account_number, "service_address": service_address,
		               "bill_file": bill_file_base64,
		               "service_city": service_city, "wireless_pin": wireless_pin, "caller_id": caller_id,
		               "signature": signature_base64,
		               "service_state": service_state, "service_zip": service_zip, "btn": btn,
...
def change_account_status_of_customer(request):
	if request.is_ajax():
		...
		if CustomerAccountStatus.objects.filter(customer_id=request.POST['id']).exists():
			...
			if customer[0]['contract_sent'] == 'False' and customer[0]['contract_accepted'] == 'False' and customer[0][
				'first_payment_done'] == 'False' and customer[0]['req_has_been_sent'] == 'False' and customer[0][
				'devices_shipped'] == 'False' and customer[0]['devices_received'] == 'False' and customer[0][
				'installation_date'] == 'False' and customer[0]['installation_date'] == 'False' and customer[0][
			...
			elif customer[0]['contract_sent'] == 'True' and customer[0]['contract_accepted'] == 'False' and customer[0][
				'first_payment_done'] == 'False' and customer[0]['req_has_been_sent'] == 'False' and customer[0][
				'devices_shipped'] == 'False' and customer[0]['devices_received'] == 'False' and customer[0][
				'installation_date'] == 'False' and customer[0]['installation_done'] == 'False':
			...
			elif customer[0]['contract_sent'] == 'True' and customer[0]['contract_accepted'] == 'True' and customer[0][
				'first_payment_done'] == 'False' and customer[0]['req_has_been_sent'] == 'False' and customer[0][
				'devices_shipped'] == 'False' and customer[0]['devices_received'] == 'False' and customer[0][
				'installation_date'] == 'False' and customer[0]['installation_done'] == 'False':
				...
				'first_payment_done'] == 'True' and customer[0]['req_has_been_sent'] == 'False' and customer[0][
				'devices_shipped'] == 'False' and customer[0]['devices_received'] == 'False' and customer[0][
				'installation_date'] == 'False' and customer[0]['installation_done'] == 'False':
				...
				'first_payment_done'] == 'True' and customer[0]['req_has_been_sent'] == 'True' and customer[0][
				'devices_shipped'] == 'False' and customer[0]['devices_received'] == 'False' and customer[0][
				'installation_date'] == 'False' and customer[0]['installation_done'] == 'False':
				...
				'first_payment_done'] == 'True' and customer[0]['req_has_been_sent'] == 'True' and customer[0][
				'devices_shipped'] == 'True' and customer[0]['devices_received'] == 'False' and customer[0][
				'installation_date'] == 'False' and customer[0]['installation_done'] == 'False':
				...
				'first_payment_done'] == 'True' and customer[0]['req_has_been_sent'] == 'True' and customer[0][
				'devices_shipped'] == 'True' and customer[0]['devices_received'] == 'True' and customer[0][
				'installation_date'] == 'False' and customer[0]['installation_done'] == 'False':
				...
				'first_payment_done'] == 'True' and customer[0]['req_has_been_sent'] == 'True' and customer[0][
				'devices_shipped'] == 'True' and customer[0]['devices_received'] == 'True' and customer[0][
				'installation_date'] == 'True' and customer[0]['installation_done'] == 'False':
				...
				'first_payment_done'] == 'True' and customer[0]['req_has_been_sent'] == 'True' and customer[0][
				'devices_shipped'] == 'True' and customer[0]['devices_received'] == 'True' and customer[0][
				'installation_date'] == 'True' and customer[0]['installation_done'] == 'True':
...
def send_password_sms(request, pk):
	if request.is_ajax():
		...
		# send sms
		msg = 'Your Password has been changed. Your new password is - {}'.format(customer_obj.portal_password)
...
class SendCustomSMS(View):
	def get(self, request, pk):
		customer_obj = Customer.objects.get(id=pk)
		return render(request, 'admin/users/custom_sms.html', {'customer_obj': customer_obj})
